...
(* The carrier (foo X Y Z) of the super-powerset associated to a
   ...
   types X and Y would be naturally defined in Coq as follows:
     ...
     | foo_in : (x:X; y:Y)(Rel X)->X->(foo X Y Z)
     | foo_rt : (foo X Y Z).
...
(*** Building the edge relation of the syper-powerset ***)

(* The edge relation (FOO X Y Z) : (Rel (foo X Y Z)) of the super-
   powerset of the family of small types Z : X->Y->Typ1 is defined
